...
class OsmNode_DirectionTestCase: XCTestCase {
	...
	func testDirectionTagValueFormatsBearingWhenNoDirectionTagExists() {
		let node = OsmNode(asUserCreated: "")

		XCTAssertEqual(node.directionTagValue(forBearingDegrees: 0), "0")
		XCTAssertEqual(node.directionTagValue(forBearingDegrees: 370), "10")
	}
